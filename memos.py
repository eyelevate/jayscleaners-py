...
class Memo:
    ...
    def create_table(self):
        table_schema = ', '.join([PrimaryKeyField(column='id').data_type(),
                                  IntegerField(column='memo_id').data_type(),
                                  IntegerField(column='company_id').data_type(),
                                  CharField(column='memo', max_length=200).data_type(),
    ...
    def add(self):
        ...
        self.c.execute('''INSERT INTO {t}(memo_id,company_id,memo,ordered,status,created_at,updated_at)
VALUES(?,?,?,?,?,?,?)'''.format(t=table), (self.memo_id,
                                           self.company_id,
                                           self.memo,
                                           self.ordered,
                                           self.status,
    ...
    def update(self):
        ...
        self.c.execute('''UPDATE {t} SET memo_id = ?, company_id = ?, memo = ?, ordered = ?, status = ?, updated_at = ?
WHERE id = ?'''.format(t=table), (self.memo_id,
                                  self.company_id,
                                  self.memo,
                                  self.ordered,
                                  self.status,
